...
export const prog1Content = {
  'variables-types': {
    ...
    intro: [
      { en: 'The type decides how many bytes the value occupies, what operations are allowed, and how it is interpreted. An <code>int</code> and a <code>double</code> of the same bit pattern mean completely different numbers.', de: 'Der Typ bestimmt, wie viele Bytes der Wert belegt, welche Operationen erlaubt sind und wie er interpretiert wird. Ein <code>int</code> und ein <code>double</code> mit gleichem Bitmuster bedeuten voellig verschiedene Zahlen.', sl: 'Tip določa, koliko bajtov vrednost zaseda, katere operacije so dovoljene in kako se razlaga. <code>int</code> in <code>double</code> z enakim bitnim vzorcem pomenita popolnoma različni števili.' },
    ],
    how: [
      { en: 'Common primitive types: whole numbers (<code>int</code>), real numbers (<code>double</code>), single characters (<code>char</code>), truth values (<code>bool</code>), and text (<code>string</code>).', de: 'Gaengige primitive Typen: ganze Zahlen (<code>int</code>), reelle Zahlen (<code>double</code>), einzelne Zeichen (<code>char</code>), Wahrheitswerte (<code>bool</code>) und Text (<code>string</code>).', sl: 'Pogosti primitivni tipi: cela števila (<code>int</code>), realna števila (<code>double</code>), posamezni znaki (<code>char</code>), logične vrednosti (<code>bool</code>) in besedilo (<code>string</code>).' },
      { en: 'A local variable lives on the <strong>stack</strong> and disappears when its block ends. Static typing catches type errors before the program ever runs.', de: 'Eine lokale Variable liegt auf dem <strong>Stack</strong> und verschwindet am Blockende. Statische Typisierung faengt Typfehler ab, bevor das Programm laeuft.', sl: 'Lokalna spremenljivka živi na <strong>skladu</strong> in izgine, ko se njen blok konča. Statična tipizacija ujame tipske napake, še preden se program zažene.' },
    ],
    code: {
      cpp: `int age = 20;\ndouble price = 9.99;\nchar grade = 'A';\nstd::string name = "Ann";\nbool ok = true;`,
  ...
  'functions': {
    ...
    pitfalls: [
      { en: 'Passing a large object <strong>by value</strong> copies it every call. In C++ pass by <code>const&amp;</code> to avoid the copy without allowing changes.', de: 'Ein grosses Objekt <strong>per Wert</strong> zu uebergeben kopiert es bei jedem Aufruf. In C++ per <code>const&amp;</code> uebergeben, um die Kopie ohne Aenderungsrecht zu vermeiden.', sl: 'Podajanje velikega objekta <strong>po vrednosti</strong> ga kopira ob vsakem klicu. V C++ ga podajajte po <code>const&amp;</code>, da se izognete kopiji brez pravice do spreminjanja.' },
    ],
  },

  'pointers': {
    tldr: { en: 'A <strong>pointer</strong> is a variable that stores a <strong>memory address</strong>. Through it you read or change the value living at that address, and you walk the heap, arrays and linked structures. A <strong>reference</strong> is a safer alias for an existing variable. This is a C and C++ signature; managed languages hide it.', de: 'Ein <strong>Zeiger</strong> ist eine Variable, die eine <strong>Speicheradresse</strong> haelt. Ueber ihn liest oder aenderst du den Wert an dieser Adresse und durchlaeufst Heap, Felder und verkettete Strukturen. Eine <strong>Referenz</strong> ist ein sicherer Alias fuer eine bestehende Variable. Typisch fuer C und C++; verwaltete Sprachen verbergen es.', sl: '<strong>Kazalec</strong> je spremenljivka, ki hrani <strong>pomnilniški naslov</strong>. Prek njega berete ali spreminjate vrednost na tem naslovu ter se sprehajate po kopici, poljih in povezanih strukturah. <strong>Referenca</strong> je varnejši vzdevek za obstoječo spremenljivko. To je značilnost jezikov C in C++; upravljani jeziki jo skrivajo.' },
    intro: [
    ...
    pitfalls: [
      { en: 'A <strong>dangling pointer</strong> (pointing at freed memory) and a <strong>null dereference</strong> are the classic crashes. After <code>delete</code>, set the pointer to <code>nullptr</code>.', de: 'Ein <strong>haengender Zeiger</strong> (auf freigegebenen Speicher) und eine <strong>Null-Dereferenzierung</strong> sind die klassischen Abstuerze. Nach <code>delete</code> den Zeiger auf <code>nullptr</code> setzen.', sl: '<strong>Viseči kazalec</strong> (kaže na sproščen pomnilnik) in <strong>razrešitev ničelnega kazalca</strong> sta klasična sesutja. Po <code>delete</code> nastavite kazalec na <code>nullptr</code>.' },
  ...
  'recursion': {
    tldr: { en: '<strong>Recursion</strong> is a function that calls itself on a smaller input, with a <strong>base case</strong> that stops the descent. Each call gets its own stack frame. It shines on self-similar problems: factorials, tree walks, divide-and-conquer.', de: '<strong>Rekursion</strong> ist eine Funktion, die sich mit kleinerer Eingabe selbst aufruft, mit einem <strong>Basisfall</strong>, der den Abstieg stoppt. Jeder Aufruf hat seinen eigenen Stapelrahmen. Sie glaenzt bei selbstaehnlichen Problemen: Fakultaeten, Baumdurchlaeufe, Teile-und-herrsche.', sl: '<strong>Rekurzija</strong> je funkcija, ki kliče samo sebe na manjšem vhodu, z <strong>osnovnim primerom</strong>, ki ustavi sestop. Vsak klic dobi svoj skladovni okvir. Blesti pri samopodobnih problemih: fakultete, sprehodi po drevesih, deli in vladaj.' },
    intro: [
      { en: 'Two parts are mandatory: the <strong>base case</strong> (the smallest input, answered directly) and the <strong>recursive case</strong> (reduce the problem and call yourself). Miss the base case and the calls never stop, overflowing the stack.', de: 'Zwei Teile sind Pflicht: der <strong>Basisfall</strong> (kleinste Eingabe, direkt beantwortet) und der <strong>Rekursionsfall</strong> (Problem verkleinern und sich selbst aufrufen). Fehlt der Basisfall, enden die Aufrufe nie und der Stapel laeuft ueber.', sl: 'Obvezna sta dva dela: <strong>osnovni primer</strong> (najmanjši vhod, odgovorjen neposredno) in <strong>rekurzivni primer</strong> (zmanjšaj problem in pokliči samega sebe). Če izpustiš osnovni primer, se klici nikoli ne ustavijo in sklad se prelije.' },
    ],
    how: [
      { en: 'Every recursion has an equivalent <strong>iterative</strong> version using an explicit stack; recursion just lets the call stack do that bookkeeping for you.', de: 'Jede Rekursion hat eine gleichwertige <strong>iterative</strong> Version mit explizitem Stapel; Rekursion laesst nur den Aufrufstapel diese Buchhaltung uebernehmen.', sl: 'Vsaka rekurzija ima enakovredno <strong>iterativno</strong> različico z eksplicitnim skladom; rekurzija zgolj prepusti to knjigovodstvo klicnemu skladu.' },
    ],
    code: {
