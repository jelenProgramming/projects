...
export const webContent = {
  'css': {
    ...
    intro: [
      { en: 'When two rules target the same element, the <strong>cascade</strong> decides the winner by <strong>specificity</strong> (an id beats a class beats an element), then by source order. Some properties (like <code>color</code> and <code>font</code>) also <strong>inherit</strong> from parent to child.',
        de: 'Zielen zwei Regeln auf dasselbe Element, entscheidet die <strong>Kaskade</strong> nach <strong>Spezifitaet</strong> (id schlaegt Klasse schlaegt Element), dann nach Reihenfolge. Manche Eigenschaften (wie <code>color</code>, <code>font</code>) <strong>vererben</strong> sich zudem vom Eltern- zum Kindelement.' },
      { en: 'Every element is a rectangular box: <strong>content</strong> at the centre, then <strong>padding</strong>, then a <strong>border</strong>, then <strong>margin</strong> outside. <code>box-sizing: border-box</code> makes width include padding and border, which is far easier to reason about.',
        de: 'Jedes Element ist ein Rechteck: <strong>Inhalt</strong> in der Mitte, dann <strong>Innenabstand</strong>, dann <strong>Rahmen</strong>, dann <strong>Aussenabstand</strong>. <code>box-sizing: border-box</code> laesst die Breite Innenabstand und Rahmen einschliessen, was das Rechnen stark vereinfacht.' },
    ],
    how: [
      ...
      { en: '<strong>Flexbox</strong> lays out items in one dimension (a row or a column) and is ideal for toolbars and centring.',
        de: '<strong>Flexbox</strong> ordnet Elemente in einer Dimension (Zeile oder Spalte) an, ideal fuer Leisten und Zentrieren.' },
      { en: '<strong>Grid</strong> lays out in two dimensions (rows and columns at once), ideal for page and card layouts. <strong>Media queries</strong> adapt the design to screen width.',
        de: '<strong>Grid</strong> ordnet in zwei Dimensionen an (Zeilen und Spalten zugleich), ideal fuer Seiten- und Kartenlayouts. <strong>Media Queries</strong> passen das Design an die Bildschirmbreite an.' },
    ],
    pitfalls: [
      { en: 'Fighting specificity with ever-more-specific selectors (or <code>!important</code>) makes stylesheets unmaintainable. Keep selectors flat and lean on classes.',
        de: 'Spezifitaet mit immer spezifischeren Selektoren (oder <code>!important</code>) zu bekaempfen macht Stylesheets unwartbar. Halte Selektoren flach und setze auf Klassen.' },
    ],
  },
  ...
  'javascript': {
    tldr: {
      ...
      de: '<strong>JavaScript</strong> ist die Sprache im Browser. Sie ist dynamisch typisiert. <strong>Objekte</strong> gruppieren Daten und Verhalten als Schluessel-Wert-Paare; <strong>Prototypen</strong> ermoeglichen Vererbung; und ES6-<strong>Klassen</strong> sind syntaktischer Zucker ueber diesen Prototypen.',
    },
    intro: [
      ...
      { en: 'The <code>class</code> keyword gives a cleaner way to build objects with shared methods and <code>extends</code> for inheritance, but under the hood it still uses prototypes.',
        de: 'Das Schluesselwort <code>class</code> bietet eine sauberere Art, Objekte mit geteilten Methoden und <code>extends</code> fuer Vererbung zu bauen, verwendet intern aber weiterhin Prototypen.' },
    ],
    how: [
      ...
      { en: '<strong>class / extends:</strong> declare fields and methods once; a subclass reuses and overrides them.',
        de: '<strong>class / extends:</strong> Felder und Methoden einmal deklarieren; eine Unterklasse nutzt und ueberschreibt sie.' },
    ],
    pitfalls: [
      { en: 'Use <code>===</code> (strict equality), not <code>==</code>, which coerces types and gives surprises like <code>0 == ""</code> being true.',
        de: 'Nutze <code>===</code> (strenge Gleichheit), nicht <code>==</code>, das Typen umwandelt und Ueberraschungen wie <code>0 == ""</code> gleich wahr liefert.' },
      { en: 'The value of <code>this</code> depends on how a function is called, not where it is defined. Arrow functions capture the surrounding <code>this</code>.',
        de: 'Der Wert von <code>this</code> haengt davon ab, wie eine Funktion aufgerufen wird, nicht wo sie definiert ist. Pfeilfunktionen uebernehmen das umgebende <code>this</code>.' },
    ],
  },
  ...
  'async-js': {
    tldr: {
      ...
      de: 'JavaScript laeuft in einem <strong>einzigen Thread</strong>, langsame Arbeit (Netz, Timer) darf ihn also nicht blockieren. Die <strong>Ereignisschleife</strong> fuehrt eingereihte Rueckrufe aus, sobald der Stapel frei ist. Die Werkzeuge entwickelten sich von <strong>Callbacks</strong> ueber <strong>Promises</strong> zu <strong>async/await</strong>.',
    },
    intro: [
      { en: 'A <strong>callback</strong> is a function you hand to an async operation to run when it finishes. Nesting many of them produces unreadable "callback hell".',
        de: 'Ein <strong>Callback</strong> ist eine Funktion, die du einer asynchronen Operation gibst, damit sie nach dem Ende laeuft. Viele davon zu verschachteln erzeugt die unlesbare Callback-Hoelle.' },
      { en: 'A <strong>Promise</strong> is an object representing a future value, in one of three states: <strong>pending</strong>, <strong>fulfilled</strong>, or <strong>rejected</strong>. You chain with <code>.then()</code>/<code>.catch()</code>, or write it linearly with <code>async</code>/<code>await</code>.',
        de: 'Ein <strong>Promise</strong> ist ein Objekt fuer einen zukuenftigen Wert in einem von drei Zustaenden: <strong>ausstehend</strong>, <strong>erfuellt</strong> oder <strong>abgelehnt</strong>. Du verkettest mit <code>.then()</code>/<code>.catch()</code> oder schreibst es linear mit <code>async</code>/<code>await</code>.' },
    ],
    how: [
      { en: '<strong>async</strong> marks a function that returns a Promise; <strong>await</strong> pauses inside it until a Promise settles, without blocking the whole page.',
        de: '<strong>async</strong> markiert eine Funktion, die ein Promise zurueckgibt; <strong>await</strong> pausiert darin, bis ein Promise sich klaert, ohne die ganze Seite zu blockieren.' },
    ...
    pitfalls: [
      { en: 'Forgetting to <code>await</code> (or return) a Promise means the code moves on before the work is done, a very common bug.',
        de: 'Ein vergessenes <code>await</code> (oder return) bedeutet, dass der Code weiterlaeuft, bevor die Arbeit fertig ist, ein sehr haeufiger Fehler.' },
    ],
  },
  ...
  'web-security': {
    tldr: {
      ...
      de: 'Die Web-Angriffe, gegen die jeder Entwickler schuetzen muss: <strong>XSS</strong> (Skript in eine Seite einschleusen), <strong>SQL-Injection</strong> (SQL in eine Abfrage einschleusen) und <strong>CSRF</strong> (einen angemeldeten Nutzer zu einer gefaelschten Anfrage verleiten). Die goldene Regel: <strong>traue nie der Client-Eingabe</strong>.',
    },
    intro: [
      { en: '<strong>XSS (Cross-Site Scripting):</strong> if user text is put into the page without escaping, an attacker can inject <code>&lt;script&gt;</code> that runs in other visitors&#39; browsers and steals their session.',
        de: '<strong>XSS (Cross-Site-Scripting):</strong> wird Nutzertext ungeschuetzt in die Seite gesetzt, kann ein Angreifer <code>&lt;script&gt;</code> einschleusen, das in den Browsern anderer laeuft und ihre Sitzung stiehlt.' },
      { en: '<strong>SQL injection:</strong> building a query by string-concatenating user input lets an attacker change its meaning (the classic <code>&#39; OR 1=1 --</code>).',
        de: '<strong>SQL-Injection:</strong> eine Abfrage per String-Verkettung aus Nutzereingabe zu bauen laesst einen Angreifer ihre Bedeutung aendern (der Klassiker <code>&#39; OR 1=1 --</code>).' },
    ],
    how: [
    ...
    pitfalls: [
      { en: 'Validating input only in the browser is no protection: the server must re-check everything, because an attacker bypasses the page entirely.',
        de: 'Eingaben nur im Browser zu pruefen schuetzt nicht: der Server muss alles erneut pruefen, denn ein Angreifer umgeht die Seite ganz.' },
    ],
  },
  ...
  'web-frameworks': {
    ...
    intro: [
      { en: 'Server-side frameworks usually follow <strong>MVC</strong>: the <strong>model</strong> holds data, the <strong>view</strong> renders it, the <strong>controller</strong> handles requests. They bundle routing, database access, templating, sessions and security defaults.',
        de: 'Serverseitige Frameworks folgen meist <strong>MVC</strong>: das <strong>Modell</strong> haelt Daten, die <strong>Ansicht</strong> stellt sie dar, der <strong>Controller</strong> bearbeitet Anfragen. Sie buendeln Routing, Datenbankzugriff, Templates, Sitzungen und Sicherheitsvorgaben.' },
      { en: 'Client-side frameworks keep a component tree and re-render only what changed, giving app-like interactivity. Many apps combine both: an API on the server, a single-page app on the client.',
        de: 'Clientseitige Frameworks halten einen Komponentenbaum und rendern nur das Geaenderte neu, was app-artige Interaktivitaet gibt. Viele Apps verbinden beides: eine API auf dem Server, eine Single-Page-App auf dem Client.' },
    ],
    how: [
    ...
    pitfalls: [
      { en: 'A framework saves time but adds a learning curve and lock-in; for a tiny static page, plain HTML/CSS/JS is often the better choice.',
        de: 'Ein Framework spart Zeit, bringt aber Lernkurve und Bindung; fuer eine winzige statische Seite ist schlichtes HTML/CSS/JS oft die bessere Wahl.' },
    ],
  },
}
