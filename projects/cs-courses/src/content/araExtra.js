  ...
  'arithmetic-transforms': {
    ...
    defs: [
      { name: { en: 'E1 (lower half)', de: 'E1 (untere Haelfte)' },
        body: { en: 'When the whole interval sits in the lower half [0, $\\tfrac{1}{2}$): the leading bit is settled as 0. Output 0, then double the interval: low = 2∗low, high = 2∗high.', de: 'Wenn das ganze Intervall in der unteren Haelfte [0, $\\tfrac{1}{2}$) liegt: das fuehrende Bit ist 0. 0 ausgeben, dann verdoppeln: low = 2∗low, high = 2∗high.' },
        formal: { tex: 'high < \\tfrac{1}{2} \\implies \\text{emit } 0;\\; [low,high) \\to [2\\,low,\\, 2\\,high)' } },
      { name: { en: 'E2 (upper half)', de: 'E2 (obere Haelfte)' },
        body: { en: 'When the whole interval sits in the upper half [$\\tfrac{1}{2}$, 1): the leading bit is settled as 1. Output 1, then rescale: low = 2∗low - 1, high = 2∗high - 1.', de: 'Wenn das ganze Intervall in der oberen Haelfte [$\\tfrac{1}{2}$, 1) liegt: das fuehrende Bit ist 1. 1 ausgeben, dann reskalieren: low = 2∗low - 1, high = 2∗high - 1.' },
      ...
      { name: { en: 'E3 (straddles the middle)', de: 'E3 (ueberspannt die Mitte)' },
        body: { en: 'When the interval straddles $\\tfrac{1}{2}$ but is trapped in the middle quarter [$\\tfrac{1}{4}$, $\\tfrac{3}{4}$): we cannot yet output a bit. Rescale around $\\tfrac{1}{2}$ and remember a pending/underflow count to resolve later: low = 2∗low - $\\tfrac{1}{2}$, high = 2∗high - $\\tfrac{1}{2}$.', de: 'Wenn das Intervall die $\\tfrac{1}{2}$ ueberspannt, aber im mittleren Viertel [$\\tfrac{1}{4}$, $\\tfrac{3}{4}$) gefangen ist: noch kein Bit ausgebbar. Um $\\tfrac{1}{2}$ reskalieren und einen Underflow-Zaehler merken: low = 2∗low - $\\tfrac{1}{2}$, high = 2∗high - $\\tfrac{1}{2}$.' },
    ...
    intro: [
      ...
      { en: '<strong>E1</strong> and <strong>E2</strong> handle the easy cases: once the interval is wholly in one half, its top bit can never change, so emit it and zoom in. <strong>E3</strong> is the subtle one: the interval hugs the middle, so the next bit is undecided, but it is shrinking dangerously. E3 rescales around $\\tfrac{1}{2}$ and increments an <em>underflow counter</em>; when the next real bit b is finally emitted (by E1 or E2), you also emit that many opposite bits (1-b).', de: '<strong>E1</strong> und <strong>E2</strong> erledigen die einfachen Faelle: liegt das Intervall ganz in einer Haelfte, kann das oberste Bit sich nie mehr aendern - ausgeben und hineinzoomen. <strong>E3</strong> ist der knifflige: das Intervall umklammert die Mitte, das naechste Bit ist unbestimmt, schrumpft aber gefaehrlich. E3 reskaliert um $\\tfrac{1}{2}$ und erhoeht einen <em>Underflow-Zaehler</em>; wird das naechste echte Bit b ausgegeben, folgen ebenso viele Gegenbits (1-b).' },
    ],
    paper: [
      { en: 'After each symbol narrows [low, high), check the three conditions in a loop until none applies: high &lt; $\\tfrac{1}{2}$ → E1 (emit 0), low ≥ $\\tfrac{1}{2}$ → E2 (emit 1), or [$\\tfrac{1}{4}$, $\\tfrac{3}{4}$) straddle → E3 (count++).', de: 'Nach jeder Symbol-Verengung von [low, high) die drei Bedingungen in einer Schleife pruefen, bis keine gilt: high &lt; $\\tfrac{1}{2}$ → E1 (0 ausgeben), low ≥ $\\tfrac{1}{2}$ → E2 (1 ausgeben), oder [$\\tfrac{1}{4}$, $\\tfrac{3}{4}$)-Ueberspannung → E3 (Zaehler++).' },
