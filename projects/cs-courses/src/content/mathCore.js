...
export const mathCoreContent = {
  'warshall': {
    tldr: { en: 'Computes the <strong>transitive closure</strong> of a graph: can i reach j (possibly through other nodes)? Same triple loop as Floyd-Warshall but with booleans instead of distances. <strong>O(V³)</strong>.', de: 'Berechnet die <strong>transitive Hülle</strong> eines Graphen: kann i j erreichen (evtl. über andere Knoten)? Gleiche Dreifachschleife wie Floyd-Warshall, aber boolesch statt Distanzen. <strong>O(V³)</strong>.' },
    when: { en: 'Reachability queries, detecting whether a relation is transitive, dependency analysis, and equivalence/closure problems in discrete math. Use when you need yes/no connectivity, not shortest distance.', de: 'Erreichbarkeitsabfragen, Transitivitätsprüfung einer Relation, Abhängigkeitsanalyse und Hüllen-Probleme in der diskreten Mathematik.' },
    how: [
      { en: 'Start with the adjacency matrix (1 where a direct edge exists).', de: 'Mit der Adjazenzmatrix beginnen (1 bei direkter Kante).' },
      { en: 'For each intermediate node k, for every pair (i,j): if i→k and k→j are both reachable, mark i→j reachable.', de: 'Für jeden Zwischenknoten k, für jedes Paar (i,j): sind i→k und k→j erreichbar, markiere i→j als erreichbar.' },
      { en: 'After all k, R[i][j]=1 means j is reachable from i.', de: 'Nach allen k bedeutet R[i][j]=1, dass j von i erreichbar ist.' },
    ],
    paper: [
      { en: 'Write the matrix. For each k (outer), scan row i and column k: wherever R[i][k]=1, OR row k into row i.', de: 'Matrix schreiben. Pro k (aussen) Zeile i und Spalte k scannen: wo R[i][k]=1, Zeile k in Zeile i ODER-verknüpfen.' },
      { en: 'Do k = A, then B, then C... in order. k is always the outermost loop - the same rule as Floyd-Warshall.', de: 'k = A, dann B, dann C... der Reihe nach. k ist stets die äusserste Schleife - dieselbe Regel wie Floyd-Warshall.' },
    ],
    complexity: [
      { case: { en: 'All cases', de: 'Alle Fälle' }, time: 'O(V³)', space: 'O(V²)', cls: 'avg', note: { en: 'Three nested loops over all nodes; k must be the outer loop, exactly like Floyd-Warshall.', de: 'Drei verschachtelte Schleifen; k muss aussen sein, genau wie Floyd-Warshall.' } },
    ],
    pitfalls: [
      { en: 'Warshall = reachability (boolean); Floyd-Warshall = shortest distances (numbers). Same structure, different update.', de: 'Warshall = Erreichbarkeit (boolesch); Floyd-Warshall = kürzeste Distanzen (Zahlen). Gleiche Struktur, andere Aktualisierung.' },
    ],
  },
  'euclid-gcd': {
    tldr: { en: 'Finds the greatest common divisor by repeatedly replacing (a,b) with (b, a mod b) until the remainder is 0. <strong>O(log min(a,b))</strong> - astonishingly fast, and one of the oldest algorithms.', de: 'Findet den grössten gemeinsamen Teiler, indem (a,b) wiederholt durch (b, a mod b) ersetzt wird, bis der Rest 0 ist. <strong>O(log min(a,b))</strong>.' },
    when: { en: 'Simplifying fractions, modular arithmetic, cryptography (RSA key math), and as a subroutine in the extended version for modular inverses.', de: 'Brüche kürzen, modulare Arithmetik, Kryptographie (RSA), und als Teilroutine der erweiterten Version für modulare Inverse.' },
    how: [
    ...
    complexity: [
      { case: { en: 'All cases', de: 'Alle Fälle' }, time: 'O(log min(a,b))', space: 'O(1)', cls: 'best', note: { en: 'Worst case is consecutive Fibonacci numbers - that is where it takes the most steps.', de: 'Schlechtester Fall sind aufeinanderfolgende Fibonacci-Zahlen - dort braucht es die meisten Schritte.' } },
    ],
    pitfalls: [
  ...
  'modular-exp': {
    ...
    how: [
      { en: 'Read the exponent in binary.', de: 'Den Exponenten binär lesen.' },
    ...
    complexity: [
      { case: { en: 'All cases', de: 'Alle Fälle' }, time: 'O(log e)', space: 'O(1)', cls: 'best', note: { en: 'log e squarings vs e naive multiplications - the difference between feasible and impossible for crypto-size exponents.', de: 'log e Quadrierungen statt e Multiplikationen - der Unterschied zwischen machbar und unmöglich.' } },
    ],
    pitfalls: [
      { en: 'Reduce mod m after EVERY multiply, not just at the end, or the numbers overflow.', de: 'Nach JEDER Multiplikation mod m, nicht erst am Ende, sonst laufen die Zahlen über.' },
    ],
  },
  'extended-euclid': {
    tldr: { en: 'Euclid that also finds integers x, y with <strong>ax + by = gcd(a,b)</strong> (Bezout). Its main use: computing the <strong>modular inverse</strong> needed for RSA decryption. O(log min(a,b)).', de: 'Euklid, der zusätzlich ganze x, y mit <strong>ax + by = ggT(a,b)</strong> findet (Bezout). Hauptnutzen: das <strong>modulare Inverse</strong> für RSA.' },
    ...
    how: [
      { en: 'Run Euclid, but carry coefficients backward.', de: 'Euklid ausführen, aber Koeffizienten rückwärts mitführen.' },
      { en: 'At the base case gcd = a, the coefficients are (1, 0).', de: 'Im Basisfall ggT = a sind die Koeffizienten (1, 0).' },
      { en: 'Substitute back up the chain to express the gcd as ax + by.', de: 'Die Kette hochsubstituieren, um den ggT als ax + by auszudrücken.' },
    ],
    complexity: [
      { case: { en: 'All cases', de: 'Alle Fälle' }, time: 'O(log min(a,b))', space: 'O(log n)', cls: 'best', note: { en: 'Same speed as plain Euclid; the modular inverse exists only when gcd(a,m)=1.', de: 'So schnell wie einfacher Euklid; das modulare Inverse existiert nur bei ggT(a,m)=1.' } },
    ],
    pitfalls: [
  ...
  'crt': {
    ...
    how: [
      { en: 'Given x ≡ aᵢ (mod mᵢ) for pairwise-coprime mᵢ, let M = product of all mᵢ.', de: 'Gegeben x ≡ aᵢ (mod mᵢ) für paarweise teilerfremde mᵢ, sei M = Produkt aller mᵢ.' },
      { en: 'For each i compute Mᵢ = M/mᵢ and its inverse mod mᵢ.', de: 'Für jedes i Mᵢ = M/mᵢ und sein Inverses mod mᵢ berechnen.' },
      { en: 'Sum aᵢ∗Mᵢ∗(Mᵢ⁻¹) and take mod M for the unique answer.', de: 'aᵢ∗Mᵢ∗(Mᵢ⁻¹) summieren und mod M für die eindeutige Antwort.' },
    ],
    complexity: [
      { case: { en: 'All cases', de: 'Alle Fälle' }, time: 'O(k log M)', space: 'O(k)', cls: 'avg', note: { en: 'k = number of congruences. The moduli MUST be pairwise coprime for a unique solution.', de: 'k = Anzahl Kongrünzen. Die Moduln MUESSEN paarweise teilerfremd sein.' } },
    ],
    pitfalls: [
      { en: 'No unique solution if the moduli are not coprime - the headline condition.', de: 'Keine eindeutige Lösung, wenn die Moduln nicht teilerfremd sind - die zentrale Bedingung.' },
    ],
  },
  'combinatorics': {
    tldr: { en: 'Counting without listing. Built on two rules - <strong>sum</strong> (disjoint choices add) and <strong>product</strong> (independent steps multiply) - giving permutations P(n,k)=n!/(n-k)! (order matters) and combinations C(n,k)=n!/(k!(n-k)!) (order doesn\'t). Plus the pigeonhole principle.', de: 'Zählen ohne Auflisten. Zwei Regeln - <strong>Summe</strong> (disjunkte Wahlen addieren) und <strong>Produkt</strong> (unabhängige Schritte multiplizieren) - ergeben Permutationen P(n,k)=n!/(n-k)! und Kombinationen C(n,k)=n!/(k!(n-k)!). Plus Schubfachprinzip.' },
    intro: [
      { en: '<strong>Sum rule</strong>: if sets A₁...Aₖ are pairwise disjoint, |A₁ ∪ ... ∪ Aₖ| = |A₁| + ... + |Aₖ|. <strong>Product rule</strong>: for a sequence of independent choices, the totals multiply.', de: '<strong>Summenregel</strong>: sind A₁...Aₖ paarweise disjunkt, dann |A₁ ∪ ... ∪ Aₖ| = |A₁| + ... + |Aₖ|. <strong>Produktregel</strong>: bei unabhängigen Wahlen multiplizieren sich die Anzahlen.' },
    ...
    how: [
      ...
      { en: 'Generalized pigeonhole: with n items in m boxes, some box holds at least ⌈n/m⌉ items.', de: 'Verallgemeinert: bei n Objekten in m Fächern enthält ein Fach mindestens ⌈n/m⌉ Objekte.' },
    ],
    complexity: [
    ...
    pitfalls: [
      { en: 'Permutation vs combination: "a 3-person podium from 8" (order matters) is P(8,3); "a 3-person committee from 8" (order doesn\'t) is C(8,3). Same numbers, different answer.', de: 'Permutation vs Kombination: „3er-Podium aus 8" (Reihenfolge zählt) ist P(8,3); „3er-Komitee aus 8" ist C(8,3).' },
  ...
  'induction': {
    tldr: { en: 'A proof technique: show a statement holds for a base case (n=1), then show that if it holds for n it holds for n+1. Together these prove it for <strong>all</strong> n - like dominoes falling.', de: 'Eine Beweistechnik: zeige die Aussage für einen Basisfall (n=1), dann dass aus n auch n+1 folgt. Zusammen beweist das alle n - wie fallende Dominosteine.' },
    ...
    how: [
      { en: 'Base case: prove the statement for the smallest n (often 0 or 1).', de: 'Basisfall: die Aussage für das kleinste n beweisen (oft 0 oder 1).' },
      { en: 'Inductive hypothesis: assume it holds for some n = k.', de: 'Induktionsannahme: annehmen, sie gilt für ein n = k.' },
      { en: 'Inductive step: using that assumption, prove it for n = k+1.', de: 'Induktionsschritt: mit dieser Annahme für n = k+1 beweisen.' },
    ],
    complexity: [
      { case: { en: 'Proof technique', de: 'Beweistechnik' }, time: '-', space: '-', cls: 'best', note: { en: 'Strong induction assumes the statement for all values ≤ k, not just k - useful for recurrences.', de: 'Starke Induktion nimmt die Aussage für alle Werte ≤ k an, nicht nur k - nützlich für Rekurrenzen.' } },
    ],
    pitfalls: [
      { en: 'A missing or wrong base case invalidates the whole proof - the most common error.', de: 'Ein fehlender oder falscher Basisfall macht den ganzen Beweis ungültig - der häufigste Fehler.' },
    ],
  },
  'logic-proofs': {
    tldr: { en: 'Propositional logic: statements combined with ¬, ∧, ∨, ⇒, ⇔, judged by truth tables. The named <strong>inference rules</strong> (Modus ponens, Modus tollens, syllogisms) are valid argument patterns you cite by name in proofs.', de: 'Aussagenlogik: Aussagen verknüpft mit ¬, ∧, ∨, ⇒, ⇔, beurteilt per Wahrheitstabelle. Die benannten <strong>Schlussregeln</strong> (Modus ponens, Modus tollens, Syllogismen) sind gültige Argumentmuster.' },
    intro: [
    ...
    how: [
      ...
      { en: 'To prove an argument valid, show the implication (premises ⇒ conclusion) is a tautology.', de: 'Ein Argument als gültig beweisen: zeige, dass (Prämissen ⇒ Schluss) eine Tautologie ist.' },
    ],
    complexity: [
      { case: { en: 'Truth table', de: 'Wahrheitstabelle' }, time: '2^n rows', space: '-', cls: 'avg', note: { en: 'Key equivalences to memorize: A⇒B ≡ ¬A∨B, De Morgan ¬(A∧B) ≡ ¬A∨¬B, contrapositive A⇒B ≡ ¬B⇒¬A.', de: 'Wichtige Äquivalenzen: A⇒B ≡ ¬A∨B, De Morgan ¬(A∧B) ≡ ¬A∨¬B, Kontraposition A⇒B ≡ ¬B⇒¬A.' } },
    ],
    pitfalls: [
      { en: 'Implication with a false premise is TRUE ("if 2+2=5 then I am king" is a true statement). A⇒B fails only when A is true and B false.', de: 'Implikation mit falscher Prämisse ist WAHR. A⇒B scheitert nur bei A wahr, B falsch.' },
  ...
  'relations': {
    tldr: { en: 'A relation R on a set A is any set of ordered pairs from A×A. Its <strong>properties</strong> classify it: reflexive, symmetric, transitive (and more). The three together define an <strong>equivalence relation</strong>, which partitions A into disjoint classes.', de: 'Eine Relation R auf einer Menge A ist eine Menge geordneter Paare aus A×A. Ihre <strong>Eigenschaften</strong> klassifizieren sie: reflexiv, symmetrisch, transitiv. Alle drei zusammen definieren eine <strong>Äquivalenzrelation</strong>, die A in disjunkte Klassen teilt.' },
    intro: [
      { en: 'A <strong>relation</strong> R ⊆ A×A relates element a to b when (a,b) ∈ R, written aRb. The exam tests whether a given relation has each property - here are the exact definitions:', de: 'Eine <strong>Relation</strong> R ⊆ A×A verbindet a mit b, wenn (a,b) ∈ R, geschrieben aRb. Die Prüfung testet, ob eine Relation jede Eigenschaft hat - hier die exakten Definitionen:' },
      { en: '<strong>Reflexive</strong>: aRa for every a ∈ A.   <strong>Irreflexive</strong>: never aRa.   <strong>Symmetric</strong>: aRb ⟹ bRa.   <strong>Asymmetric</strong>: aRb ⟹ ¬bRa.   <strong>Antisymmetric</strong>: (aRb ∧ bRa) ⟹ a = b.   <strong>Transitive</strong>: (aRb ∧ bRc) ⟹ aRc.', de: '<strong>Reflexiv</strong>: aRa für jedes a ∈ A.   <strong>Irreflexiv</strong>: nie aRa.   <strong>Symmetrisch</strong>: aRb ⟹ bRa.   <strong>Asymmetrisch</strong>: aRb ⟹ ¬bRa.   <strong>Antisymmetrisch</strong>: (aRb ∧ bRa) ⟹ a = b.   <strong>Transitiv</strong>: (aRb ∧ bRc) ⟹ aRc.' },
      { en: 'An <strong>equivalence relation</strong> is reflexive, symmetric AND transitive. It splits A into <strong>equivalence classes</strong> - disjoint subsets where everything is related (e.g. "has the same remainder mod 3" splits the integers into 3 classes). A <strong>partial order</strong> is reflexive, antisymmetric and transitive (e.g. ≤, or subset ⊆).', de: 'Eine <strong>Äquivalenzrelation</strong> ist reflexiv, symmetrisch UND transitiv. Sie teilt A in <strong>Äquivalenzklassen</strong>. Eine <strong>Halbordnung</strong> ist reflexiv, antisymmetrisch und transitiv (z.B. ≤ oder ⊆).' },
    ],
    paper: [
      { en: 'To check a property, test its definition against every relevant pair. Reflexive: is (a,a) present for ALL a? One missing pair kills it.', de: 'Eine Eigenschaft prüfen: ihre Definition an jedem Paar testen. Reflexiv: ist (a,a) für ALLE a da? Ein fehlendes Paar widerlegt es.' },
    ...
    pitfalls: [
      ...
      { en: 'Transitivity is vacuously true if there are no chains a→b→c to check. "No counterexample" = it holds.', de: 'Transitivität gilt leer, wenn es keine Ketten a→b→c gibt. „Kein Gegenbeispiel" = es gilt.' },
    ],
  },
  ...
  'epsilon-delta': {
    defs: [
      { name: { en: 'Limit (ε-δ definition)', de: 'Grenzwert (ε-δ-Definition)' },
        body: { en: 'L is the limit of f at c if f(x) can be made arbitrarily close to L by taking x close enough to c (but not equal to c).', de: 'L ist der Grenzwert von f bei c, wenn f(x) beliebig nahe an L gebracht werden kann, indem man x nahe genug an c wählt (aber ungleich c).' },
      ...
      { name: { en: 'One-sided limits', de: 'Einseitige Grenzwerte' },
        body: { en: 'The two-sided limit exists only if the left limit and the right limit exist and agree.', de: 'Der beidseitige Grenzwert existiert nur, wenn linker und rechter Grenzwert existieren und übereinstimmen.' },
    ...
    how: [
      { en: 'Claim: lim(x→c) f(x) = L means f(x) can be forced arbitrarily close to L.', de: 'Behauptung: lim(x→c) f(x) = L heisst, f(x) lässt sich beliebig nah an L zwingen.' },
      ...
      { en: 'You pick the ε (how close to L); the challenge is producing a δ (how close to c) that works.', de: 'Du wählst ε (Nähe zu L); die Aufgabe ist ein passendes δ (Nähe zu c).' },
    ],
    complexity: [
      { case: { en: 'Definition', de: 'Definition' }, time: '-', space: '-', cls: 'best', note: { en: 'The order matters: ε is given FIRST, then you find δ. Swapping them is the classic misunderstanding.', de: 'Die Reihenfolge zählt: ε kommt ZUERST, dann findest du δ.' } },
    ],
    pitfalls: [
      { en: 'ε comes first, δ depends on it - not the other way around. This is THE thing to internalize.', de: 'ε zürst, δ hängt davon ab - nicht umgekehrt. DAS muss sitzen.' },
    ],
  },
  'derivative': {
    defs: [
    ...
    when: { en: 'Rates of change: velocity, growth, slopes, optimization (max/min where f\'=0). The single most-used tool in applied math, physics and ML.', de: 'Änderungsraten: Geschwindigkeit, Wachstum, Steigungen, Optimierung (Max/Min bei f\'=0).' },
    how: [
      ...
      { en: 'As h → 0 the secant becomes the tangent; its slope is the derivative f\'(x).', de: 'Für h → 0 wird die Sekante zur Tangente; ihre Steigung ist die Ableitung f\'(x).' },
  ...
  'riemann': {
    defs: [
      { name: { en: 'Definite integral (Riemann)', de: 'Bestimmtes Integral (Riemann)' },
        body: { en: 'The definite integral is the limit of Riemann sums as the partition width goes to zero - the signed area between f and the x-axis.', de: 'Das bestimmte Integral ist der Grenzwert der Riemann-Summen, wenn die Streifenbreite gegen null geht - die vorzeichenbehaftete Fläche.' },
    ...
    when: { en: 'The definite integral = area under a curve = accumulated total (distance from speed, charge from current). The other half of calculus, inverse to the derivative.', de: 'Das bestimmte Integral = Fläche unter der Kurve = akkumulierte Summe. Die andere Hälfte der Analysis, invers zur Ableitung.' },
    how: [
      { en: 'Slice [a,b] into n strips of width Δx.', de: '[a,b] in n Streifen der Breite Δx schneiden.' },
      { en: 'Approximate the area as a sum of rectangle areas Σ f(xᵢ)∗Δx.', de: 'Die Fläche als Summe der Rechteckflächen Σ f(xᵢ)∗Δx nähern.' },
      { en: 'As n → ∞ (thinner strips) the sum converges to the exact area - the definite integral.', de: 'Für n → ∞ konvergiert die Summe zur exakten Fläche - das bestimmte Integral.' },
    ],
    complexity: [
    ...
    pitfalls: [
      { en: 'Area below the x-axis counts as negative in a definite integral.', de: 'Fläche unter der x-Achse zählt im bestimmten Integral negativ.' },
    ],
  },
  'taylor': {
    defs: [
      { name: { en: 'Taylor series', de: 'Taylor-Reihe' },
        body: { en: 'A function is approximated near a point a by a polynomial built from its derivatives at a. Centered at a = 0 it is called a Maclaurin series.', de: 'Eine Funktion wird nahe einem Punkt a durch ein Polynom aus ihren Ableitungen bei a angenähert. Um a = 0 heisst sie Maclaurin-Reihe.' },
        formal: { tex: 'f(x) = \\sum_{n=0}^{\\infty} \\dfrac{f^{(n)}(a)}{n!}\\,(x-a)^n' } },
    ],
    when: { en: 'Approximating hard functions (sin, eˣ, ln) with polynomials a computer can actually evaluate. Behind calculators, physics approximations, and numerical methods.', de: 'Schwierige Funktionen (sin, eˣ, ln) mit Polynomen nähern, die ein Computer auswerten kann.' },
    how: [
      ...
      { en: 'Each added term (using the next derivative / k!) improves the fit over a wider range.', de: 'Jeder weitere Term (nächste Ableitung / k!) verbessert die Näherung über einen grösseren Bereich.' },
    ...
    complexity: [
      { case: { en: 'Concept', de: 'Konzept' }, time: '-', space: '-', cls: 'best', note: { en: 'A Maclaurin series is just a Taylor series centered at 0. More terms = wider accurate range.', de: 'Eine Maclaurin-Reihe ist eine Taylor-Reihe um 0. Mehr Terme = grösserer genauer Bereich.' } },
    ],
    pitfalls: [
  ...
  'newton': {
    defs: [
      { name: { en: 'Newton iteration', de: 'Newton-Iteration' },
        body: { en: 'Each step replaces the current guess with where its tangent line crosses the x-axis.', de: 'Jeder Schritt ersetzt die Schätzung durch den Schnittpunkt ihrer Tangente mit der x-Achse.' },
    ...
    when: { en: 'Solving equations with no closed form, optimization (finding where the derivative is 0), square roots, and numerical libraries everywhere.', de: 'Gleichungen ohne geschlossene Form lösen, Optimierung, Quadratwurzeln und überall in numerischen Bibliotheken.' },
    how: [
      { en: 'Start from a guess x₀.', de: 'Mit einer Schätzung x₀ beginnen.' },
    ...
    complexity: [
      { case: { en: 'Convergence', de: 'Konvergenz' }, time: 'quadratic', space: 'O(1)', cls: 'best', note: { en: 'Doubles correct digits per step near the root - but a poor starting guess or f\'=0 can make it diverge.', de: 'Verdoppelt korrekte Stellen pro Schritt nahe der Nullstelle - aber schlechte Schätzung oder f\'=0 kann divergieren.' } },
    ],
    pitfalls: [
  ...
  'sequences': {
    ...
    tldr: { en: 'A sequence is an ordered list (a₁, a₂, ...); a series is their sum. The central question is <strong>convergence</strong>: does it approach a finite limit, or grow without bound?', de: 'Eine Folge ist eine geordnete Liste; eine Reihe ihre Summe. Die zentrale Frage ist <strong>Konvergenz</strong>: nähert sie sich einem endlichen Grenzwert?' },
    ...
    complexity: [
      { case: { en: 'Geometric series', de: 'Geometrische Reihe' }, time: 'Σrⁿ = 1/(1-r)', space: '-', cls: 'best', note: { en: 'Converges iff |r|<1. The harmonic series Σ1/n diverges even though its terms → 0 - a famous trap.', de: 'Konvergiert gdw |r|<1. Die harmonische Reihe Σ1/n divergiert, obwohl die Glieder → 0 - eine berühmte Falle.' } },
    ],
    pitfalls: [
  ...
  'mean-value': {
    ...
    when: { en: 'These are existence theorems - they guarantee a point exists without telling you to compute much. They underpin Taylor\'s theorem, error bounds, and proofs that a function is constant/increasing. On exams you cite them by name to justify a step.', de: 'Existenzsätze - sie garantieren einen Punkt, ohne viel Rechnung. Sie stützen Taylors Satz, Fehlerschranken und Monotoniebeweise. In Prüfungen zitierst du sie namentlich.' },
    how: [
      { en: 'Check the hypotheses first: continuous on the CLOSED interval [a,b], differentiable on the OPEN interval (a,b).', de: 'Zürst die Voraussetzungen prüfen: stetig auf dem ABGESCHLOSSENEN [a,b], differenzierbar auf dem OFFENEN (a,b).' },
    ...
    pitfalls: [
      { en: 'The hypotheses are not optional: drop differentiability at even one interior point and the conclusion can fail (e.g. |x|).', de: 'Die Voraussetzungen sind nicht optional: fällt die Differenzierbarkeit an nur einem inneren Punkt weg, kann der Schluss scheitern (z.B. |x|).' },
      { en: 'The theorems say c EXISTS, not that it is unique - there may be several such points.', de: 'Die Sätze sagen, c EXISTIERT, nicht dass es eindeutig ist - es kann mehrere geben.' },
    ],
  },

  // ---- DP / DIVIDE read pages ----
  'optimal-bst': {
    tldr: { en: 'Given keys with known search frequencies, build the BST that <strong>minimizes total expected search cost</strong> - frequent keys end up near the root. Solved with O(n³) dynamic programming.', de: 'Bei Schlüsseln mit bekannten Suchhäufigkeiten den BST bauen, der die <strong>erwarteten Suchkosten minimiert</strong>. Per O(n³) dynamischer Programmierung.' },
    ...
    how: [
      { en: 'Try every key as the root of every sub-range.', de: 'Jeden Schlüssel als Wurzel jedes Teilbereichs versuchen.' },
      { en: 'The cost of a tree = root frequency + cost of optimal left and right subtrees.', de: 'Baumkosten = Wurzelhäufigkeit + Kosten der optimalen Teilbäume.' },
      { en: 'Fill a DP table over increasing range sizes, keeping the cheapest root for each range.', de: 'Eine DP-Tabelle über wachsende Bereiche füllen, je billigste Wurzel behalten.' },
    ],
    complexity: [
      { case: { en: 'Standard DP', de: 'Standard-DP' }, time: 'O(n³)', space: 'O(n²)', cls: 'avg', note: { en: 'Knuth\'s optimization brings it to O(n²). Distinct from a balanced BST, which ignores frequencies.', de: 'Knuths Optimierung bringt es auf O(n²). Anders als ein balancierter BST, der Häufigkeiten ignoriert.' } },
    ],
    pitfalls: [
      { en: 'Optimal ≠ balanced: a skewed tree can be optimal if the frequent keys sit shallow.', de: 'Optimal ≠ balanciert: ein schiefer Baum kann optimal sein, wenn häufige Schlüssel flach liegen.' },
    ],
  },
  'knapsack': {
    tldr: { en: 'Choose items (each with a weight and value) to maximize value within a weight limit. <strong>0/1 knapsack</strong> (take or leave each) is solved by O(nW) DP; the greedy approach works only for the fractional version.', de: 'Gegenstände (Gewicht + Wert) wählen, um den Wert unter einer Gewichtsgrenze zu maximieren. <strong>0/1-Rucksack</strong> per O(nW) DP.' },
    ...
    how: [
      { en: 'DP table: best value using the first i items within weight limit w.', de: 'DP-Tabelle: bester Wert mit den ersten i Gegenständen bei Limit w.' },
      { en: 'For each item, choose the better of: skip it, or take it (value + best for the remaining weight).', de: 'Pro Gegenstand das Bessere wählen: überspringen oder nehmen (Wert + Bestes für Restgewicht).' },
    ...
    complexity: [
      { case: { en: '0/1 (DP)', de: '0/1 (DP)' }, time: 'O(n∗W)', space: 'O(n∗W)', cls: 'avg', note: { en: 'O(nW) is "pseudo-polynomial" - it depends on the numeric capacity W, not just n. 0/1 knapsack is NP-hard.', de: 'O(nW) ist „pseudopolynomiell" - hängt von der Kapazität W ab, nicht nur n. 0/1-Rucksack ist NP-schwer.' } },
    ],
    pitfalls: [
      { en: 'Greedy (best value/weight first) is OPTIMAL for fractional knapsack but WRONG for 0/1 - the key distinction.', de: 'Greedy ist OPTIMAL für den fraktionalen, aber FALSCH für den 0/1-Rucksack - die zentrale Unterscheidung.' },
    ],
  },
  'tsp': {
    tldr: { en: 'Find the shortest route visiting every city exactly once and returning home. <strong>NP-hard</strong>: brute force is O(n!), the Held-Karp DP is O(n²∗2ⁿ). For large n you use heuristics, not exact answers.', de: 'Die kürzeste Rundreise über alle Städte finden. <strong>NP-schwer</strong>: Brute Force O(n!), Held-Karp-DP O(n²∗2ⁿ).' },
    ...
    how: [
      { en: 'Exact (Held-Karp): DP over subsets of visited cities + current city.', de: 'Exakt (Held-Karp): DP über Teilmengen besuchter Städte + aktuelle Stadt.' },
      { en: 'Branch-and-bound prunes routes that already exceed the best found.', de: 'Branch-and-Bound beschneidet Routen, die das beste Ergebnis schon überschreiten.' },
      { en: 'Heuristics (nearest-neighbor, 2-opt, Christofides) give good-enough routes fast.', de: 'Heuristiken (Nächster-Nachbar, 2-opt, Christofides) liefern schnell gute Routen.' },
    ],
    complexity: [
    ...
    pitfalls: [
      { en: 'There is no known efficient exact solution - if you find one, you have solved P vs NP.', de: 'Es gibt keine bekannte effiziente exakte Lösung - fändest du eine, hättest du P vs NP gelöst.' },
    ],
  },
  'strassen': {
    ...
    how: [
      { en: 'Split each matrix into four n/2 × n/2 blocks.', de: 'Jede Matrix in vier n/2 × n/2 Blöcke teilen.' },
      ...
      { en: 'Recursing with 7 instead of 8 changes the exponent from 3 to log₂7 ≈ 2.81.', de: 'Rekursion mit 7 statt 8 ändert den Exponenten von 3 auf log₂7 ≈ 2.81.' },
    ],
    complexity: [
    ...
    pitfalls: [
      { en: 'The extra additions and recursion overhead mean it only beats the naive method for fairly large n.', de: 'Der Zusatzaufwand bedeutet, dass es erst für recht grosse n schneller ist.' },
    ],
  },
  'master-theorem': {
    ...
    when: { en: 'Analyzing any recursive divide-and-conquer algorithm: merge sort, binary search, Strassen, Karatsuba. The fastest way to a complexity answer on an exam.', de: 'Analyse jedes rekursiven Teile-und-herrsche-Algorithmus: Merge Sort, binäre Suche, Strassen, Karatsuba.' },
    how: [
      ...
      { en: 'Case 1: f smaller → T = Θ(n^(log_b a)). Case 2: equal → Θ(n^(log_b a)∗log n). Case 3: f larger → Θ(f(n)).', de: 'Fall 1: f kleiner → Θ(n^(log_b a)). Fall 2: gleich → Θ(n^(log_b a)∗log n). Fall 3: f grösser → Θ(f(n)).' },
    ],
    complexity: [
      { case: { en: 'Merge sort: a=2,b=2,f=n', de: 'Merge Sort: a=2,b=2,f=n' }, time: 'Θ(n log n)', space: '-', cls: 'best' },
      { case: { en: 'Binary search: a=1,b=2', de: 'Binäre Suche: a=1,b=2' }, time: 'Θ(log n)', space: '-', cls: 'best', note: { en: 'Compare f(n) to n^(log_b a) - that comparison is the whole theorem.', de: 'f(n) mit n^(log_b a) vergleichen - dieser Vergleich ist das ganze Theorem.' } },
    ],
    pitfalls: [
